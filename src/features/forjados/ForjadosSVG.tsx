...
export function ForjadosSVG({ inp, result, section, width, mode = 'screen' }: Props) {
  ...
  // Common color tokens
  const bgFill       = isPdf ? '#f1f5f9' : 'var(--color-bg-surface)';
  const strokeBorder = isPdf ? '#334155' : 'var(--color-border-main)';
  const strokeBar    = isPdf ? '#0f172a' : 'var(--color-text-primary)';
  const strokeAccent = isPdf ? '#0ea5e9' : 'var(--color-accent)';   // neutral axis + cotas
