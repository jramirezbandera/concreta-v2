...
export async function exportMicropilesPDF(
  ...
  // pintaban sin medir y desbordaban su columna — p.ej. el valor de la
  // capacidad geotécnica (`CR = 8.00 (E1 arena floja (fuera de tabla))`) se
  // comía Ut% y Estado. La descripción, además, se truncaba en silencio
  // (`splitTextToSize(...)[0]`, sin elipsis). Mismo arreglo que retainingWall.
  ...
  // La regla superior va 3.2 mm POR ENCIMA de la línea base: a cuerpo 6 el
  // texto sube ~2.1 mm, así que la anterior (`y - 0.5`) caía DENTRO de las
  // letras y tachaba la cabecera.
