...
export const MAX_HISTORY_TURNS = 12;    // ventana deslizante (6 pares) al construir la request
 ...
 * - Ventana de MAX_HISTORY_TURNS podando SIEMPRE por pares (user+assistant)
 *   desde el principio → alternancia estricta y primer turno user.
 ...
 *   IMAGE_OMITTED_MARKER en línea aparte al final.
 */
export function buildChatTurns(items: ReadonlyArray<ChatItemLike>): ChatTurn[] {
  ...
  if (turns.length > MAX_HISTORY_TURNS) {
    const excess = turns.length - MAX_HISTORY_TURNS;
