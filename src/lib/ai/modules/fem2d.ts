...
import { solveFem2D, type Fem2DAnalysisResult } from '../../../features/fem2d/pipeline';
...
const ROLES = ['auto', 'pilar', 'viga', 'cordon', 'diagonal', 'montante'] as const;
...
export const FEM2D_PAYLOAD_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    nudos: {
      type: ['array', 'null'],
      description: 'Lista COMPLETA de nudos (REEMPLAZA la actual entera; null = sin cambio). Coordenadas en METROS, ejes del mundo: +x derecha, +y ARRIBA. El nudo k de esta lista es el "nudo k" al que apuntan barras, apoyos y cargas (1-based). Conserva el ORDEN de los nudos existentes y añade los nuevos AL FINAL. Si cambias nudos, revisa también "barras" y "apoyos".',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['x', 'y'],
        properties: {
          x: { type: 'number', description: 'Coordenada x en metros (+x derecha).' },
          y: { type: 'number', description: 'Coordenada y en metros (+y ARRIBA; la base suele ser y=0).' },
        },
      },
    },
    barras: {
      type: ['array', 'null'],
      description: 'Lista COMPLETA de barras (REEMPLAZA la actual entera; null = sin cambio). La barra k conserva la identidad de la barra k actual (posicional): dentro de una barra, un campo null = conservar su valor actual (o el default si la barra es nueva). Conserva el orden y añade las nuevas al final.',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['nudo_i', 'nudo_j', 'tipo', 'rol', 'perfil', 'acero', 'correas_m', 'rotulas'],
        properties: {
          nudo_i: { type: 'integer', description: 'Nº de nudo inicial (1-based sobre la lista final de nudos).' },
          nudo_j: { type: 'integer', description: 'Nº de nudo final (1-based).' },
          tipo: { type: ['string', 'null'], enum: [...TIPOS_BARRA, null], description: 'viga-columna = axil + flexión (lo normal); biela = solo axil (celosías), biarticulada por formulación y SIN cargas en la barra. null = conservar (viga-columna en barras nuevas).' },
          rol: { type: ['string', 'null'], enum: [...ROLES, null], description: 'Dirige la comprobación: pilar → motor de pilares; viga/cordon → motor de vigas; diagonal/montante → axil (solo bielas). "auto" = deducir de la geometría (vertical ±10° → pilar, si no viga). null = conservar.' },
          perfil: { type: ['string', 'null'], enum: [...PERFIL_NAMES, null], description: 'Perfil del catálogo (nombre EXACTO: "IPE 240", "HEB 200", "L 80×8"…). "L 80×8" solo sirve para bielas. null = conservar (o heredar de la barra anterior si es nueva).' },
          acero: { type: ['string', 'null'], enum: [...ACEROS, null], description: 'Grado del acero. null = conservar.' },
          correas_m: { type: ['number', 'null'], description: 'Separación (m) entre arriostramientos del ala comprimida (correas/viguetas/forjado) en vigas y cordones: limita la longitud de vuelco lateral (LTB). 0 = SIN arriostrar (lado seguro). null = conservar. Solo redúcela si el usuario confirma que esas correas existen.' },
          rotulas: { type: ['string', 'null'], enum: [...ROTULAS, null], description: 'Rótula (liberación de momento) en los extremos de la barra: "i" (en nudo_i), "j" (en nudo_j), "ambas" o "ninguna". Solo viga-columna: una biela ya es biarticulada por formulación. null = conservar las actuales.' },
        },
      },
    },
    apoyos: {
      type: ['array', 'null'],
      description: 'Lista COMPLETA de apoyos (REEMPLAZA la actual entera; null = sin cambio). Un elemento por nudo apoyado; los nudos no listados quedan libres.',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['nudo', 'tipo'],
        properties: {
          nudo: { type: 'integer', description: 'Nº de nudo apoyado (1-based sobre la lista final de nudos).' },
          tipo: { type: 'string', enum: [...APOYOS], description: 'articulado (fijo en x e y), empotrado (además impide el giro), deslizante (solo impide y).' },
        },
      },
    },
    cargas: {
      type: ['array', 'null'],
      description: 'Lista COMPLETA de cargas (REEMPLAZA la actual entera; [] = sin cargas; null = sin cambio). Componentes CON SIGNO: +x derecha, +y ARRIBA ⇒ la gravedad es fy NEGATIVO (10 kN hacia abajo = fy: -10). Valores CARACTERÍSTICOS sin mayorar. Conserva el orden de las existentes y añade las nuevas al final.',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['tipo', 'objetivo', 'fx', 'fy', 'ejes', 'pos', 'desde', 'hasta', 'hipotesis', 'categoria_uso'],
        properties: {
          tipo: { type: 'string', enum: [...CARGA_TIPOS], description: 'nudo (kN en un nudo), repartida (kN/m sobre una barra), puntual_barra (kN dentro de una barra).' },
          objetivo: { type: 'integer', description: 'Índice 1-based: nº de NUDO (tipo nudo) o nº de BARRA (repartida, puntual_barra) sobre las listas finales.' },
          fx: { type: 'number', description: 'Componente x CON SIGNO: kN (nudo, puntual_barra) o kN/m (repartida). Viento hacia la derecha = positivo.' },
          fy: { type: 'number', description: 'Componente y CON SIGNO: kN o kN/m. GRAVEDAD = NEGATIVO (hacia abajo). Succión de viento = positivo.' },
          ejes: { type: 'string', enum: [...EJES], description: 'global = ejes del mundo (lo normal); local = ejes de la barra (x_local de nudo_i a nudo_j, +y_local a su izquierda) para presiones ⊥, p. ej. viento sobre un faldón. Las cargas de nudo son siempre globales.' },
          pos: { type: ['number', 'null'], description: 'Posición relativa 0–1 desde nudo_i. OBLIGATORIA en puntual_barra; null en el resto.' },
          desde: { type: ['number', 'null'], description: 'Inicio relativo 0–1 de una repartida PARCIAL. null = 0 (todo el largo).' },
          hasta: { type: ['number', 'null'], description: 'Fin relativo 0–1 de una repartida PARCIAL. null = 1.' },
          hipotesis: { type: 'string', enum: [...HIPOTESIS], description: 'G permanente, Q sobrecarga de uso, W viento, S nieve, E sismo. La app aplica γ, ψ y las combinaciones.' },
...
export function projectModel2D(m: Fem2DModel): ModelProj {
  ...
  const nudos: NudoProj[] = m.nodes.map((n) => ({ x: round2(n.x), y: round2(n.y) }));
  ...
  const barras: BarraProj[] = m.members.map((mm) => ({
    ...
    rol: mm.role,
    perfil: mm.material === 'steel' && mm.steelSelection !== undefined
    ...
    rotulas: mm.elementType === 'beam-column'
      ? (mm.releases.i && mm.releases.j ? 'ambas' : mm.releases.i ? 'i' : mm.releases.j ? 'j' : 'ninguna')
      : 'ninguna',
  }));
  ...
  const apoyos: ApoyoProj[] = m.supports
    .map((s) => ({ nudo: nodeIdx.get(s.node) ?? 0, tipo: SUPPORT_TO_APOYO[s.type] }))
    .filter((a) => a.nudo > 0)
  ...
  for (const l of m.loads) {
    ...
    if (l.kind === 'node') {
      ...
      cargas.push({
        tipo: 'nudo', objetivo: k, fx: round2(l.Fx), fy: round2(l.Fy), ejes: 'global',
        pos: null, desde: null, hasta: null, hipotesis: l.lc, categoria_uso: categoria,
      });
    } else if (l.kind === 'udl') {
      const k = memberIdx.get(l.member);
      if (k === undefined) continue;
      cargas.push({
        tipo: 'repartida', objetivo: k, fx: round2(l.wx), fy: round2(l.wy), ejes: l.frame,
      ...
      if (k === undefined) continue;
      cargas.push({
        tipo: 'puntual_barra', objetivo: k, fx: round2(l.Fx), fy: round2(l.Fy), ejes: l.frame,
        pos: l.pos, desde: null, hasta: null, hipotesis: l.lc, categoria_uso: categoria,
      });
    }
  }
...
function buildTemplateDefaults<P>(t: { defaults: () => P; build: (p: P) => Fem2DModel }): Fem2DModel {
...
function presetBaseline(current: Fem2DModel): Fem2DModel | null {
  ...
  switch (current.templateId) {
    case 'pratt-truss': return buildTemplateDefaults(FEM2D_TEMPLATES['pratt-truss']);
    case 'portal-frame': return buildTemplateDefaults(FEM2D_TEMPLATES['portal-frame']);
    case 'multistory': return buildTemplateDefaults(FEM2D_TEMPLATES['multistory']);
    case 'gable': return buildTemplateDefaults(FEM2D_TEMPLATES['gable']);
    default: return null;
  }
}
...
const KEY_ORDER: readonly PayloadKey[] = ['nudos', 'barras', 'apoyos', 'cargas', 'peso_propio'];
...
function fmtNudos(nudos: readonly NudoProj[]): string {
  return `${nudos.length} nudo${nudos.length === 1 ? '' : 's'}: `
...
function fmtCarga(c: CargaProj2D): string {
  ...
  if (c.tipo === 'nudo') return `(${c.fx}, ${c.fy}) kN en nudo ${c.objetivo} ${hip}`;
  if (c.tipo === 'repartida') {
...
export const CARGAS2D_ELEMENT_RULES: ReadonlyArray<ElementSafetyRule<CargaProj2D>> = [
  {
...
export const CARGAS2D_RISK_CTX = {
  ...
  removalWhy: 'Las acciones las fija el proyecto: eliminar cargas reduce la demanda de toda la estructura. Confírmalo solo si esas cargas realmente ya no existen.',
...
export const BARRAS_RISK_CTX = {
  ...
  removalWhy: 'Quitar barras reescribe la estructura y elimina con ellas sus cargas: confírmalo solo si la geometría real ha cambiado.',
  identityKeys: [] as const,
} as const;
...
export const NUDOS_RISK_CTX = {
  ...
  removalWhy: 'Quitar nudos elimina en cascada las barras que los tocan y sus cargas: confírmalo solo si la geometría real ha cambiado.',
  identityKeys: [] as const,
} as const;
...
function mapNudos(
  ...
  for (let k = 0; k < items.length; k++) {
    const it = items[k];
    const n = k + 1;
    if (it.x === null || it.y === null) return `Nudo ${n}: faltan coordenadas x/y`;
    ...
    const id = k < current.nodes.length ? current.nodes[k].id : mintId('n', used);
    out.push({ id, x, y });
  }
  return out;
}
...
function mapBarras(
  ...
  for (let k = 0; k < items.length; k++) {
    ...
    // --- Nudos ---
    if (it.nudo_i === null || !Number.isInteger(it.nudo_i)
      || it.nudo_j === null || !Number.isInteger(it.nudo_j)) {
      return `Barra ${n}: nudo_i/nudo_j ausentes o no enteros`;
    }
    if (it.nudo_i < 1 || it.nudo_i > finalNodes.length
      || it.nudo_j < 1 || it.nudo_j > finalNodes.length) {
      return `Barra ${n}: nudo ${it.nudo_i < 1 || it.nudo_i > finalNodes.length ? it.nudo_i : it.nudo_j} no existe (hay ${finalNodes.length})`;
    }
    if (it.nudo_i === it.nudo_j) return `Barra ${n}: nudo_i y nudo_j deben ser distintos`;
    ...
    if (dup !== undefined) return `Barra ${n}: duplica la barra ${dup} (mismos nudos ${pairKey})`;
    ...
    // --- Tipo de elemento ---
    const prevTipo: TipoBarra | null = prev !== null
      ? (prev.elementType === 'two-force' ? 'biela' : 'viga-columna')
      : null;
    ...
    if (it.rol !== null && it.rol !== 'auto') {
      if (tipo === 'biela' && ROLES_FLEXION.includes(it.rol)) {
        return `Barra ${n}: el rol '${it.rol}' no vale para una biela (usa diagonal, montante o "auto")`;
      }
      if (tipo !== 'biela' && ROLES_AXIALES.includes(it.rol)) {
        return `Barra ${n}: el rol '${it.rol}' es de biela; una viga-columna usa pilar, viga o cordon (o "auto")`;
      }
      role = it.rol;
      roleManual = true;
      ...
      role = !roleManual && (prev.role === 'pilar' || prev.role === 'viga')
        ? inferRole(a, b)
        : prev.role;
    }
    ...
    if (prev !== null && prev.material === 'rc' && it.perfil === null && it.acero === null) {
      ...
      const lastSteel = out.length > 0 && out[out.length - 1].material === 'steel'
    ...
    if (it.correas_m === null) {
      ...
      if (it.correas_m < LIMITS.correas.min || it.correas_m > LIMITS.correas.max) {
    ...
    if (tipo === 'biela') {
      if (it.correas_m !== null && it.correas_m !== 0) {
        warnings.push(`Barra ${n}: correas_m solo aplica a vigas/cordones; en una biela se ignora.`);
      }
      ltbSpacing = undefined;
    }
    ...
    if (tipo !== 'biela' && prev !== null && prev.elementType === 'beam-column') {
    ...
    if (it.rotulas !== null) {
      if (tipo === 'biela') {
        if (it.rotulas !== 'ninguna') {
          warnings.push(`Barra ${n}: una biela ya es biarticulada por formulación; "rotulas" se ignora.`);
        }
      } else {
        releases = {
...
function mapApoyos(
  ...
  if (items.length === 0) return 'La estructura necesita al menos un apoyo; la lista no puede quedar vacía';
  const out: Fem2DSupport[] = [];
  const seen = new Set<number>();
  for (let k = 0; k < items.length; k++) {
    const it = items[k];
    const n = k + 1;
    if (it.nudo === null || !Number.isInteger(it.nudo)) return `Apoyo ${n}: falta el nº de nudo`;
    if (it.nudo < 1 || it.nudo > finalNodes.length) {
      return `Apoyo ${n}: nudo ${it.nudo} no existe (hay ${finalNodes.length})`;
    }
    if (it.tipo === null) return `Apoyo ${n}: tipo no reconocido (articulado/empotrado/deslizante)`;
    if (seen.has(it.nudo)) return `Apoyo ${n}: el nudo ${it.nudo} ya tiene apoyo en esta lista`;
    seen.add(it.nudo);
    out.push({ node: finalNodes[it.nudo - 1].id, type: APOYO_TO_SUPPORT[it.tipo] });
  }
  return out;
}
...
function mapCarga2D(
  ...
  const n = index + 1;
  if (raw.tipo === null) return `Carga ${n}: tipo ausente o no reconocido (nudo/repartida/puntual_barra)`;
  if (raw.hipotesis === null) return `Carga ${n}: falta hipotesis (G/Q/W/S/E)`;
  if (raw.fx === null || raw.fy === null) return `Carga ${n}: faltan las componentes fx/fy`;

  const fx = round2(raw.fx);
  const fy = round2(raw.fy);
  if (fx === 0 && fy === 0) return `Carga ${n}: componentes nulas (fx = fy = 0)`;
  ...
  if (raw.tipo === 'nudo' && ejes === 'local') {
    warnings.push(`Carga ${n}: las cargas de nudo son siempre globales; se ignora ejes "local".`);
    ejes = 'global';
  }
  ...
  if (ejes === 'global' && fy > 0 && (raw.hipotesis === 'G' || raw.hipotesis === 'Q' || raw.hipotesis === 'S')) {
    warnings.push(`Carga ${n} (${raw.hipotesis}): componente vertical positiva (hacia ARRIBA) — revisa el signo si es una carga gravitatoria.`);
  }

  if (raw.objetivo === null || !Number.isInteger(raw.objetivo)) {
    return `Carga ${n}: objetivo ausente o no entero`;
  }
  const maxTarget = raw.tipo === 'nudo' ? finalNodes.length : finalMembers.length;
  ...
  if (target.elementType === 'two-force') {
    return `Carga ${n}: la barra ${raw.objetivo} es una biela y no admite cargas en la barra (aplícalas en los nudos o cámbiala a viga-columna)`;
  }

  if (raw.tipo === 'repartida') {
    if (raw.pos !== null) warnings.push(`Carga ${n}: "pos" solo aplica a puntual_barra; se ignora.`);
  ...
  if (raw.pos === null) return `Carga ${n}: falta pos (posición relativa 0–1 desde nudo_i)`;
  if (raw.pos < 0 || raw.pos > 1) return `Carga ${n}: pos ${raw.pos} fuera del rango [0, 1]`;
  return {
    id: mintId('l', used), kind: 'point-member', lc,
    ...(useCategory !== undefined ? { useCategory } : {}),
    member: target.id,
    pos: raw.pos,
    Fx: fx,
...
function structuralFieldsPresent(fields: Partial<Fem2DModel>): boolean {
  return fields.nodes !== undefined || fields.members !== undefined
...
function buildFem2DPlan(
  ...
  if (x.nudos !== null) {
    ...
    if (typeof res === 'string') {
      skip('nudos', `${res} — no se aplica ningún nudo (la lista reemplaza a la actual entera)`);
    } else {
  ...
  if (x.barras !== null && !handled.has('barras')) {
    if (x.barras.length === 0) {
      skip('barras', 'El modelo necesita al menos una barra; la lista no puede quedar vacía');
    } else {
      const barraWarnings: string[] = [];
      const res = mapBarras(x.barras, finalNodes, current, usedIds, barraWarnings);
      if (typeof res === 'string') {
        skip('barras', `${res} — no se aplica ninguna barra (la lista reemplaza a la actual entera)`);
      } else {
        const afterProj = projectModel2D({ ...current, nodes: finalNodes, members: res }).barras;
    ...
    finalMembers = kept;
    if (pruned > 0) {
      warnings.push(`${pruned} barra(s) eliminadas: su nudo desaparece con la nueva geometría. Al cambiar el número de nudos conviene enviar también "barras".`);
    }
  }
  ...
  if (x.apoyos !== null && !handled.has('apoyos')) {
    ...
    if (typeof res === 'string') {
      skip('apoyos', `${res} — no se aplica ningún apoyo (la lista reemplaza a la actual entera)`);
    } else {
      const afterProj = projectModel2D({ ...current, nodes: finalNodes, supports: res }).apoyos;
    ...
    if (kept.length !== current.supports.length) {
      ...
      warnings.push(`${current.supports.length - kept.length} apoyo(s) eliminados: su nudo desaparece con la nueva geometría. Al cambiar el número de nudos conviene enviar también "apoyos".`);
    }
  }
  ...
  if (x.cargas !== null && !handled.has('cargas')) {
    ...
    for (let i = 0; i < x.cargas.length; i++) {
      const res = mapCarga2D(x.cargas[i], i, finalNodes, finalMembers, usedIds, cargaWarnings);
    ...
    if (elementError !== null) {
      ...
      const afterProj = projectModel2D({
        ...current, nodes: finalNodes, members: finalMembers, loads: mapped,
      }).cargas;
      if (!nudosApplied && !barrasApplied && sameProj(curProj.cargas, afterProj)) {
        skip('cargas', ALREADY);
      } else {
    ...
    const kept = current.loads.filter((l) =>
      l.kind === 'node' ? survivingNodes.has(l.node) : survivingMembers.has(l.member));
    if (kept.length !== current.loads.length) {
      fields.loads = kept;
      warnings.push(`${current.loads.length - kept.length} carga(s) eliminadas: su barra/nudo desaparece con la nueva geometría. Revisa la lista de cargas.`);
    }
  }
  ...
  if (structuralFieldsPresent(fields)) {
    ...
    const beforeFails = new Set(
      solveFem2D(current).errors.filter((e) => e.severity === 'fail').map(failKey),
    );
  ...
  const scalarDefaults: Fem2DModel = baseline ?? { ...current, selfWeight: !current.selfWeight };
  const baseProj = baseline !== null ? projectModel2D(baseline) : null;

  const finalModel: Fem2DModel = { ...current, ...fields };
  const finalProj = structuralFieldsPresent(fields) ? projectModel2D(finalModel) : null;
  const proposedNudos = fields.nodes !== undefined && finalProj !== null ? finalProj.nudos : undefined;
  const proposedBarras = fields.members !== undefined && finalProj !== null ? finalProj.barras : undefined;
  const proposedCargas = fields.loads !== undefined && finalProj !== null ? finalProj.cargas : undefined;

  const risks = [
    ...detectSafetyRisks(FEM2D_SAFETY_RULES, changes, fields, current, scalarDefaults, confirmed),
    ...detectElementRisks(NUDOS_ELEMENT_RULES, proposedNudos, curProj.nudos, baseProj?.nudos ?? [], NUDOS_RISK_CTX, confirmed),
    ...detectElementRisks(BARRAS_ELEMENT_RULES, proposedBarras, curProj.barras, baseProj?.barras ?? [], BARRAS_RISK_CTX, confirmed),
    ...detectElementRisks(CARGAS2D_ELEMENT_RULES, proposedCargas, curProj.cargas, baseProj?.cargas ?? [], CARGAS2D_RISK_CTX, confirmed),
  ];
...
function buildSnapshot2D(c: Fem2DModel): string {
  ...
  const haPendientes = c.members
    ...
    .map(({ m, k }) => {
      const sec = m.rcSection ? `${m.rcSection.b}×${m.rcSection.h} cm HA-${m.rcSection.fck}` : 'sin sección';
 ...
 */
function reactionExtras(model: Fem2DModel, result: Fem2DAnalysisResult): string[] {
  ...
  const supports = [...model.supports]
    .sort((s1, s2) => (nodeIdx.get(s1.node) ?? 0) - (nodeIdx.get(s2.node) ?? 0));
  const parts: string[] = [];
  ...
  return [`Reacciones en apoyos (envolvente ELU, ejes del mundo, +y arriba): ${parts.join(' · ')}`];
}
...
export function summarizeFem2DResults(
  ...
  if (!result.ok || result.checks === null) {
    ...
    return summarizeCalcResults({
      valid: false,
      error: failMsgs.length > 0 ? failMsgs.join(' · ') : 'El modelo no se pudo resolver.',
      checks: [],
    });
  }
  ...
  model.members.forEach((m, k) => {
    ...
    if (verdict.status === 'pending') {
      pendingLines.push(`${desc}: PENDIENTE — ${verdict.checks.map((c) => c.val).join('; ')}`);
      return;
    }
  ...
  const extras: string[] = [`η máximo global = ${Math.round(bundle.maxEta * 100)}%`];
  if (bundle.amplified) {
    extras.push('Efectos de 2º orden: los factores de las hipótesis laterales (W/E) van amplificados por αcr (método de momentos amplificados, β=1).');
  }
