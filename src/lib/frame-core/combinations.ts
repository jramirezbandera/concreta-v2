// frame-core — Combinations module (CTE DB-SE Tabla 4.2)  [moved from
// features/fem-analysis/combinations.ts — Lane B extraction, D12]
//
// Produces weighted load combinations for ELU, ELS-frecuente and ELS-cuasiperm
// from a list of case-tagged loads. Each combination is a `WeightedSet` mapping
// loadId → factor. Solvers apply the factors to per-LC samples and aggregate
...
// have ψ0=0.7.
//
// The load parameter is the minimal structural `CaseTaggedLoad` so both the 1D
// `Load` union and the 2D `Fem2DLoad` union are accepted unchanged.

import { getPsiRow } from '../calculations/loadGen';
import type { CaseTaggedLoad, LoadCase, UseCategoryCode } from './types';
...
export function getPsi(load: CaseTaggedLoad, fallback: UseCategoryCode = 'B'): PsiRow {
...
/**
 * Build CTE Tabla 4.2 combinations from case-tagged loads.
 *
 * Algorithm:
 ...
 *
 * Effort: O(L²) where L is number of variable loads. Trivial at module scale.
 */
export function buildCombinations(loads: CaseTaggedLoad[], opts: BuildOpts = {}): ModelCombinations {
 ...
 */
export function countQLoadsWithoutCategory(loads: CaseTaggedLoad[]): number {
