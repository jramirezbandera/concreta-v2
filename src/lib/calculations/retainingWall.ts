...
//
// CE Anejo 19 §6.1         — bending resistance
// CE Anejo 19 §9.6.2   — vertical minimum reinforcement (walls)
// CE Anejo 19 §9.6.3   — horizontal minimum reinforcement (walls)
// CE Anejo 19 §9.6.4   — transverse ties (walls)
// CE Anejo 19 §9.2.1.1              — general minimum reinforcement (bending, footing)
// CE Anejo 19 §6.2.2       — shear without transverse reinforcement
...
export function calcRetainingWall(inp: RetainingWallInputs): RetainingWallResult {
  ...
  // cover is stored in mm (schema v2); wall thicknesses stay in m.
  // The < 10 mm guard doubles as a legacy trap: shared links from schema v1
  // carried cover in METERS (0.02–0.12) and must fail loudly, not compute
  // silently with a near-zero cover.
  const cover_mm   = inp.cover as number;
  if (cover_mm < 10) return invalid('Recubrimiento < 10 mm — introdúcelo en mm (p. ej. 40)');
  ...
  if (!exceedsBoundary) {
    ...
    // CE Anejo 19 §9.6.2 — vertical minimum for wall (fuste)
    ...
    // CE Anejo 19 §9.6.3 — horizontal minimum for wall (fuste), per face
    ...
    // VRd,c (CE Anejo 19 §6.2.2) — walls have no stirrups; use actual ρl when provided
    ...
    // Fuste intradós asmin — 40% of 0.002·Ac (CE Anejo 19 §9.6.2)
    if (As_prov_fv_ext > 0) {
      checks.push(makeCheck(
    ...
    // Fuste horizontal asmin — CE Anejo 19 §9.6.3 (per face)
    if (As_prov_fh > 0) {
      checks.push(makeCheck(
